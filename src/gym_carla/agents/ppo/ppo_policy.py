...
from gym_carla.agents.utils.feature_extract import XtMaCNN
from gym_carla.agents.utils.distributions import DiagGaussianDistribution
...
class PpoPolicy(nn.Module):
    ...
    def _get_features(self, birdeye: th.Tensor, state: th.Tensor) -> th.Tensor:
        """
        :param birdeye: th.Tensor (num_envs, frame_stack*channel, height, width)
        :param state: th.Tensor (num_envs, state_dim)
        """
        birdeye = birdeye.float() / 255.0
        features = self.features_extractor(birdeye, state)
        return features
    ...
    def evaluate_actions(
        self, obs_dict: Dict[str, th.Tensor], actions: th.Tensor, detach_values=False
    ):
